...
def generate_image(image, channel, time=0) -> np.ndarray:
    """
    ...
    return pixels.getPlane(0, channel, time)  # using channel number


def generate_random_image(image_list, channel: dict):
    """
    ...
    random_time = random.randint(0,random_image.getSizeT()-1)
    return random_image.getId(), random_time, generate_image(random_image, channel[1], time=random_time)
